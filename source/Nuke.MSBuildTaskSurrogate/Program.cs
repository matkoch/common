...
namespace Nuke.MSBuildTaskSurrogate
{
    public class Program
    {
        ...
        [Parameter(Separator = ";")] static string[] Files;
            ...
            try
            {
                switch (Action)
                {
                    case TaskAction.CodeGeneration:
                        CodeGenerator.GenerateCode(Files.ToList());
                        break;
                    case TaskAction.SharedFilesDownload:
                        var results = Files.Select(DownloadSharedFiles).ToList();
        ...
        private static async Task<bool> DownloadSharedFiles(string sharedFile)
        {
            try
            {
